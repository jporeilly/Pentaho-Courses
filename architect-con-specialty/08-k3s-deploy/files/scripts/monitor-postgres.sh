...
if [[ "$CONN_COUNT" -gt "${MAX_POSTGRES_CONNECTIONS}" ]]; then
    echo -e "${RED}⚠ High connection count: $CONN_COUNT (max recommended: ${MAX_POSTGRES_CONNECTIONS})${NC}"
    ISSUES=$((ISSUES + 1))
else
...
if [[ -n "$CACHE_RATIO" ]] && [[ "$CACHE_RATIO" -lt "${MIN_CACHE_HIT_RATIO}" ]]; then
    echo -e "${YELLOW}⚠ Cache hit ratio: ${CACHE_RATIO}% (recommended: >${MIN_CACHE_HIT_RATIO}%)${NC}"
