...
elif [[ "${POD_STATUS}" =~ ^(Pending|ContainerCreating|PodInitializing)$ ]]; then
    echo -e "${YELLOW}!${NC} Pentaho Server pod is starting (status: ${POD_STATUS})"
elif [[ "${POD_STATUS}" =~ ^(Error|CrashLoopBackOff|ImagePullBackOff|Failed)$ ]]; then
    echo -e "${RED}✗${NC} Pentaho Server pod has errors (status: ${POD_STATUS})"
    ((ERRORS++))
else
    echo -e "${YELLOW}!${NC} Pentaho Server pod status: ${POD_STATUS}"
fi
