...
- **Ollama** with the course's model pulled - `scriptssetup-ollama.ps1`.
- **Python 3.11+**, for the lab scripts (`pip install` steps are given in the labs).
- The **LangExtract** REST service on `localhost:8765` and the agent service on `localhost:8000`, both started from the lab materials.
- Some labs call external providers and need your own **API key**.
